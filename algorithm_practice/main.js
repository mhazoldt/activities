...
function makeCombos(comboList, item1, idx, itemList) {
  itemList.forEach(function(item2) {
    let nextCombo = [ item1, item2 ];
    comboList.push(nextCombo);
  })
  return comboList;
}
...
function arrayCombos(){
    // YOUR CODE HERE
    return singles.reduce(makeCombos, []);
}
...
function calculateFactorial(lastResult, currentNumber, idx, arr) {
  let returnValue = (idx > 0) ? lastResult * currentNumber : currentNumber;
  return returnValue;
}
...
let factorial = function( num ){
    ...
    for(let counter = 0; counter < num; counter++) {
      ...
      factorialArr.push(nextNumber);
    }
    let factorialResult = factorialArr.reduce(calculateFactorial);
...
let interpolate = function( start, end, count ){
    ...
    let intervalAmount = difference / count;
    let interpolateArr = [];

    for(counter = 0; counter < count ;counter++) {
      if(counter === 0) {
        interpolateArr.push(start)

      } else {
        let previousValue = interpolateArr[counter - 1]
        let nextValue = previousValue + intervalAmount;
        interpolateArr.push(nextValue)

      }
    }
    interpolateArr.push(end)
